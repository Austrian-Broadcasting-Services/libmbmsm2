...
static char doc[] = "libmbmsifs M2Server demo";  // NOLINT

static struct argp_option options[] = {  // NOLINT
    {"interface", 'i', "IF", 0, "IP address of the interface to bind to. Default: any (0.0.0.0 / [::])", 0},
    {"port", 'p', "PORT", 0, "Port to listen on (default: 36443)", 0},
    {"log-level", 'l', "LEVEL", 0,
 ...
 */
struct ft_arguments {
  const char *interface = {};  /**< file path of the config file. */
...
auto main(int argc, char **argv) -> int {
  ...
  spdlog::info("M2 server demo starting up");

  try {
    MbmsIfs::M2Server server(arguments.interface, arguments.port);
    for (;;)
    {
